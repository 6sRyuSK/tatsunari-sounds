...
|---|---|---|
| `Knob` | `(store, idx, theme, decimals=1)` | ロータリー。`setAccentColour(argb)`, `setNameOverride`, `setDialProfile(textTop, textBottom, nameFontPx, valueFontPx, dialInsetPx)`(<0 で既定), `requestValueEntry` |
...
`knobNeedleTip` / `fillArcBand`。visage の `flatArc` は「中心角+半開き」規約で画面角が
...
- **プラグインオーバーレイ**: `Theme::applyOverlay(json)` は現在値をシードに部分
  上書き(全キー任意)。トップレベル `"rs"` オブジェクトは共有スキーマから**無視**され、
...
- ネイティブテスト(visage 不要、非 Emscripten configure で CTest 登録):
  `factory_ui_visage_theme_test` / `factory_ui_visage_spectrum_test` / RS の
  `rs_theme_roundtrip_test`。widget の**見た目**の検証はハーネスの Playwright 側。
