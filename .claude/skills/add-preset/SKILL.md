...
- `presets/include/factory_presets/ProgramAdapter.h` — program API への委譲。
  ...
  パラメータをデフォルトへ。
- `ui/include/factory_ui/PresetSelector.h` — ComboBox + 前/次矢印の view(配色は
  `FactoryLookAndFeel` パレットのみ)。`setMenu(entries, selectedIndex)` が一般形:
  `Entry::item/header/separator` の混在行(`item` は `steppable=false` で
  prev/next 矢印の着地を禁止できる — 「Save As...」等のアクション行に使う)。
  `setItems(names, currentIndex)` は全行 item・全 steppable なフラットな旧来形の
  ショートカット。
- `ui/include/factory_ui/PresetSelectorController.h` — その view を所有し、program
  リスト反映・選択 → program API・ホスト側変更の追従(双方向同期)を一括で行う。
  **Phase 5c 以降はファクトリー program に加え、ユーザープリセット(後述)の一覧・
  ロード・Save As/Overwrite/Delete アクション行もこれ 1 つが面倒を見る**(プラグイン
  側の追加配線は不要 — `processor.getName()` からユーザープリセット保存先を自動導出)。
  Editor はこれをメンバに持つだけでよい(下の 3 参照)。
- `presets/include/factory_presets/UserPresetStore.h` — ユーザープリセットの
  ディスク永続化(`<userAppData>/TatsunariSounds/<PluginName>/Presets/<name>.xml`、
  1 プリセット 1 ファイル)。JUCE-core のみ・message-thread 専用の素朴なファイル
  I/O(list/save/load/remove/exists)。`PresetSelectorController` が内部で持つので
  プラグイン側が直接触ることは通常ない(触るのはテストの scratch ディレクトリ
  オーバーロードくらい)。ファクトリー分の本スキルの対象外(ユーザー側の保存物で
  あり `FactoryPresets.h` の管轄ではない)だが、「プリセット」を触るときは
  ファクトリー分とユーザー分の 2 系統がある、と覚えておく。
...
- **レイテンシ/CPU トレードオフ、および外部ルーティングの有効化/モニタ状態**
  (resonance-suppressor の `quality` — Fast/Normal/High はユーザーが選ぶ latency
  トレードオフで、プリセット切替がホスト PDC を勝手に再ネゴシエートしてはいけない;
  `scEnable`/`scListen` — ユーザーが配線したサイドチェインを勝手に有効化したり
  監視状態を変えてはいけない)。同種の「ユーザーが明示的に選ぶ環境設定」であって
  「プリセットが表現する音作り」ではない param は同じ理由で除外する。
...
`getStateInformation` / `setStateInformation` は共有ヘルパ
`factory_presets::stateToXml` / `applyStateXml`(`ProgramAdapter.h`)を使う。これらが
presetIndex の**追記のみ**・tag チェック・`replaceState` をまとめて行う
(`copyXmlToBinary`/`getXmlFromBinary` は `juce::AudioProcessor` の protected static
なので member 関数内に残す):

```cpp
void ...::getStateInformation (juce::MemoryBlock& d) {
    if (auto xml = factory_presets::stateToXml (apvts, programs))  // presetIndex 追記込み
        copyXmlToBinary (*xml, d);
}
void ...::setStateInformation (const void* d, int n) {
    factory_presets::applyStateXml (apvts, programs, getXmlFromBinary (d, n).get());
    // 欠落 presetIndex は 0(旧 state 互換)、tag 不一致は無視
}
```

**追記のみ = 既存セッション互換**なので version は minor で済む(major 不要)。
特殊な state(nam-player の `files` 子ツリー等)がある場合は**既存ロジックを壊さず**、
ヘルパ呼び出しの後に追加処理(例: モデル再読込)を並べる。

## 3. Editor(Source/PluginEditor.{h,cpp})

上段 26px 行(タイトル ↔ Bypass の間)にプリセットセレクタを置く。配線は
`factory_ui::PresetSelectorController`(`PresetSelectorController.h`)が全部持つ:
メンバに `factory_ui::PresetSelectorController presetController;`(processor 参照より
**後**に宣言)を足し、ctor 初期化リストで `presetController (*this, p)` と構築、
`resized()` で `presetController.selector().setBounds (...)` を配置するだけ。Editor が
`juce::AudioProcessorListener` を継承したり `refreshPresetSelector` /
`audioProcessorChanged` を手書きする必要はない(双方向同期・message thread への
marshal はコントローラ内)。既存レイアウトを壊さない(`factory-ui` スキル)。
...
1. `plugin.toml` の `version` を **minor** bump(新プリセット=新機能)。bump は
   ...
   shipped 機種も同様(次回リリースの再ビルド対象になる)。
