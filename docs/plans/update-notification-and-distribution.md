...
GUI アプリなので、OS の慣習に沿った位置に置く。

| OS | パス |
|---|---|
| macOS | `/Applications/tatsunari.app`（無ければ `~/Applications/tatsunari.app`） |
| Windows | `%LOCALAPPDATA%\Programs\tatsunari-sounds\tatsunari.exe` |

プラグインは上記を順に探索する。PATH には依存しない。
...
GUI アプリなのでターミナルは開かない。TUI を起動導線にする案（旧設計）で
問題になっていた「制御端末の確保」「ターミナルが開く違和感」は解消される。
...
1. **`curl | bash` のブートストラップを主導線として残す。** ワンライナーが
   `.app` を配置すれば隔離属性は付かず、そのまま起動できる。`.dmg` は副次的な
   導線とし、回避手順を明記する。
2. **ad-hoc 署名は必ず行う（無料）。** Apple Silicon では ad-hoc 署名が無いと
   SIGKILL される。Wails の `.app` バンドルには `codesign -s -` を明示的に当てる。
3. **回避手順を正直に案内するページを用意する**（ZL Audio と同じ方針）。
...
- Wails は CGO / OS ネイティブツールチェーンを要するため、現行の
  ...
  **macOS / Windows ランナーでのビルドに組み替える。**
- **公証は行わないため、Apple のシークレット・`notarytool`・`stapler` は不要。**
  macOS ランナーは `.app` バンドルの組み立てと `codesign -s -`（ad-hoc）にのみ
  必要。
- `.dmg` の作成も macOS ランナーで行う。
...
GUI 化に伴い、配置対象は単体バイナリではなく **`.app` バンドル**（macOS）に
なる点に注意。またブートストラップは 4.4 の緩和策 1 により、**主導線として残す**
（`.dmg` より Gatekeeper 上有利なため）。
...
2. **CI をネイティブランナーへ組み替え** — Wails の CGO 要件による。公証を
   行わないため Apple のシークレット類は不要で、組み替えは純粋にビルド環境の話。
