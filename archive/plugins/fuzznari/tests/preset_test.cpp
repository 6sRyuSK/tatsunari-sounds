//
// preset_test.cpp — wiring verification for Fuzznari's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < fuzznari_presets::kNumExclude; ++k)
            if (id == fuzznari_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (FuzznariAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (FuzznariAudioProcessor& p)
    {
    ...
    void check2_ids_exist (FuzznariAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = fuzznari_presets::bank;
    ...
    void check3_values_in_range (FuzznariAudioProcessor& p)
    {
    ...
    void check4_init_is_default (FuzznariAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (FuzznariAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<FuzznariAudioProcessor>();
    ...
    void check6_full_state_roundtrip (FuzznariAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processor = std::make_unique<FuzznariAudioProcessor>();

    std::printf ("fuzznari preset wiring (%d programs)\n", processor->getNumPrograms());

    if (processor->getNumPrograms() != 1 + fuzznari_presets::bank.numPresets)
