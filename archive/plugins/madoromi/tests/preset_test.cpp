//
// preset_test.cpp — wiring verification for madoromi's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < madoromi_presets::kNumExclude; ++k)
            if (id == madoromi_presets::kExclude[k])
                return true;
        return false;
    }
    ...
    void check1_names (MadoromiAudioProcessor& p)
    {
    ...
    void check2_ids_exist (MadoromiAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = madoromi_presets::bank;
    ...
    void check3_values_in_range (MadoromiAudioProcessor& p)
    {
    ...
    void check4_init_is_default (MadoromiAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (MadoromiAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<MadoromiAudioProcessor>();
        ...
        auto legacy = std::make_unique<MadoromiAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState);
        auto legacyRestored = std::make_unique<MadoromiAudioProcessor>();
    ...
    void check6_full_state_roundtrip (MadoromiAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<MadoromiAudioProcessor>();
    auto& processor = *processorPtr;
    std::printf ("madoromi preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + madoromi_presets::bank.numPresets)
