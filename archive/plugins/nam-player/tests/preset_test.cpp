//
// preset_test.cpp — wiring verification for the NAM Player's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < nam_player_presets::kNumExclude; ++k)
            if (id == nam_player_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (NamPlayerAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (NamPlayerAudioProcessor& p)
    {
    ...
    void check2_ids_exist (NamPlayerAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = nam_player_presets::bank;
    ...
    void check3_values_in_range (NamPlayerAudioProcessor& p)
    {
    ...
    void check4_init_is_default (NamPlayerAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (NamPlayerAudioProcessor& p)
    {
        ...
        // large inline resampler / convolver / delay buffers.
        auto restored = std::make_unique<NamPlayerAudioProcessor>();
    ...
    void check6_full_state_roundtrip (NamPlayerAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processor = std::make_unique<NamPlayerAudioProcessor>();

    std::printf ("nam-player preset wiring (%d programs)\n", processor->getNumPrograms());

    if (processor->getNumPrograms() != 1 + nam_player_presets::bank.numPresets)
