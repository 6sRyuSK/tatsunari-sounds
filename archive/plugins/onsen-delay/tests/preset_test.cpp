//
// preset_test.cpp — wiring verification for onsen-delay's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < onsen_delay_presets::kNumExclude; ++k)
            if (id == onsen_delay_presets::kExclude[k])
                return true;
        return false;
    }
    ...
    void check1_names (OnsenDelayAudioProcessor& p)
    {
    ...
    void check2_ids_exist (OnsenDelayAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = onsen_delay_presets::bank;
    ...
    void check3_values_in_range (OnsenDelayAudioProcessor& p)
    {
    ...
    void check4_init_is_default (OnsenDelayAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (OnsenDelayAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<OnsenDelayAudioProcessor>();
        ...
        auto legacy = std::make_unique<OnsenDelayAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState);
        auto legacyRestored = std::make_unique<OnsenDelayAudioProcessor>();
    ...
    void check6_full_state_roundtrip (OnsenDelayAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<OnsenDelayAudioProcessor>();
    auto& processor = *processorPtr;
    std::printf ("onsen-delay preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + onsen_delay_presets::bank.numPresets)
