//
// preset_test.cpp — wiring verification for the multiband-enhancer factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < multiband_enhancer_presets::kNumExclude; ++k)
            if (id == multiband_enhancer_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (MultibandEnhancerAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (MultibandEnhancerAudioProcessor& p)
    {
    ...
    void check2_ids_exist (MultibandEnhancerAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = multiband_enhancer_presets::bank;
    ...
    void check3_values_in_range (MultibandEnhancerAudioProcessor& p)
    {
    ...
    void check4_init_is_default (MultibandEnhancerAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (MultibandEnhancerAudioProcessor& p)
    {
    ...
    void check6_full_state_roundtrip (MultibandEnhancerAudioProcessor& p)
    {
...
int main()
{
    ...
    if (processor.getNumPrograms() != 1 + multiband_enhancer_presets::bank.numPresets)
