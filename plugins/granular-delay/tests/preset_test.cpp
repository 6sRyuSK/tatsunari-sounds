//
// preset_test.cpp — wiring verification for the granular delay's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < granular_delay_presets::kNumExclude; ++k)
            if (id == granular_delay_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (GranularDelayAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (GranularDelayAudioProcessor& p)
    {
    ...
    void check2_ids_exist (GranularDelayAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = granular_delay_presets::bank;
    ...
    void check3_values_in_range (GranularDelayAudioProcessor& p)
    {
    ...
    void check4_init_is_default (GranularDelayAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (GranularDelayAudioProcessor& p)
    {
        ...
        // Heap-allocate the extra instances: several AudioProcessors alive at
        // once on the stack overflow Windows' 1 MB thread stack (issue: preset
        // test SEGFAULT). Processors carry large inline DSP buffers.
        auto restored = std::make_unique<GranularDelayAudioProcessor>();
        ...
        auto legacy = std::make_unique<GranularDelayAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState); // deliberately no presetIndex
        auto legacyRestored = std::make_unique<GranularDelayAudioProcessor>();
    ...
    void check6_full_state_roundtrip (GranularDelayAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processor = std::make_unique<GranularDelayAudioProcessor>();

    std::printf ("granular-delay preset wiring (%d programs)\n", processor->getNumPrograms());

    if (processor->getNumPrograms() != 1 + granular_delay_presets::bank.numPresets)
