...
namespace rs_ui
{
    ...
    void RsEditor::drawBrand (visage::Canvas& canvas)
    {
        ...
        // Two-colour wordmark without measuring: draw the whole "TN Resonance
        // Suppressor" in the text colour, then overdraw the "TN" prefix in accent
        // at the same origin so its glyphs align exactly.
        canvas.setColor (visage::Color (rsTheme_.base.palette.text));
        canvas.text ("TN Resonance Suppressor", boldFont (19.0f * k()), visage::Font::kLeft, tx, r.y, tw, r.h);
        canvas.setColor (visage::Color (rsTheme_.base.palette.accent));
        canvas.text ("TN", boldFont (19.0f * k()), visage::Font::kLeft, tx, r.y, tw, r.h);
    }
