//
// ClapEntry.cpp — the tn-vocal-tuner CLAP plugin impl (IMPL_TARGET static library
...
//   * factory_presets::PresetSession       — over the tn-vocal-tuner bank + kExclude
//   * factory_shell::ClapShellPlugin<...>  — the generic CLAP glue
//
// tn-vocal-tuner is clap-first FROM BIRTH: there is no JUCE processor at all (unlike
...
//
// clap_plugin_descriptor id: jp.tatsunari-sounds.tn-vocal-tuner (reverse-DNS).
//
#include <clap/clap.h>
...
namespace
{
    ...
    const clap_plugin_descriptor_t s_desc = {
        CLAP_VERSION_INIT,
        "jp.tatsunari-sounds.tn-vocal-tuner",                    // id (reverse-DNS)
        "TN Vocal Tuner",                                   // name
    ...
    // ── the shell Policy for tn-vocal-tuner ────────────────────────────────────────
    struct PfClapPolicy
    {
