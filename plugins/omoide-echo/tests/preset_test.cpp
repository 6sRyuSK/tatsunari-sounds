//
// preset_test.cpp — wiring verification for omoide-echo's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < omoide_echo_presets::kNumExclude; ++k)
            if (id == omoide_echo_presets::kExclude[k])
                return true;
        return false;
    }
    ...
    void check1_names (OmoideEchoAudioProcessor& p)
    {
    ...
    void check2_ids_exist (OmoideEchoAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = omoide_echo_presets::bank;
    ...
    void check3_values_in_range (OmoideEchoAudioProcessor& p)
    {
    ...
    void check4_init_is_default (OmoideEchoAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (OmoideEchoAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<OmoideEchoAudioProcessor>();
        ...
        auto legacy = std::make_unique<OmoideEchoAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState);
        auto legacyRestored = std::make_unique<OmoideEchoAudioProcessor>();
    ...
    void check6_full_state_roundtrip (OmoideEchoAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<OmoideEchoAudioProcessor>();
    auto& processor = *processorPtr;
    std::printf ("omoide-echo preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + omoide_echo_presets::bank.numPresets)
