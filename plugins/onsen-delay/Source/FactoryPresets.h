...
//
// Factory presets for onsen-delay.
//
...
//
namespace onsen_delay_presets
{
    // Parameters presets must never touch. "bypass" is excluded fleet-wide so a
    // preset never silences or un-silences the plugin. "advance" is the momentary
    // sequencer-step trigger — applying it from a preset would fire a step.
    inline constexpr const char* kExclude[] = { "bypass", "advance" };
