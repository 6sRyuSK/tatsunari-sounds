//
// preset_test.cpp — wiring verification for the shimmer reverb's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < shimmer_reverb_presets::kNumExclude; ++k)
            if (id == shimmer_reverb_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (ShimmerReverbAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (ShimmerReverbAudioProcessor& p)
    {
    ...
    void check2_ids_exist (ShimmerReverbAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = shimmer_reverb_presets::bank;
    ...
    void check3_values_in_range (ShimmerReverbAudioProcessor& p)
    {
    ...
    void check4_init_is_default (ShimmerReverbAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (ShimmerReverbAudioProcessor& p)
    {
        ...
        // test SEGFAULT). The FDN reverb carries large inline delay lines.
        ...
        auto legacy = std::make_unique<ShimmerReverbAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState); // deliberately no presetIndex
        auto legacyRestored = std::make_unique<ShimmerReverbAudioProcessor>();
...
int main()
{
    ...
    auto processor = std::make_unique<ShimmerReverbAudioProcessor>();

    std::printf ("shimmer-reverb preset wiring (%d programs)\n", processor->getNumPrograms());

    if (processor->getNumPrograms() != 1 + shimmer_reverb_presets::bank.numPresets)
