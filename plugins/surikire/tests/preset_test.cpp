//
// preset_test.cpp — wiring verification for surikire's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < surikire_presets::kNumExclude; ++k)
            if (id == surikire_presets::kExclude[k])
                return true;
        return false;
    }
    ...
    void check1_names (SurikireAudioProcessor& p)
    {
    ...
    void check2_ids_exist (SurikireAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = surikire_presets::bank;
    ...
    void check3_values_in_range (SurikireAudioProcessor& p)
    {
    ...
    void check4_init_is_default (SurikireAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (SurikireAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<SurikireAudioProcessor>();
        ...
        auto legacy = std::make_unique<SurikireAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState);
        auto legacyRestored = std::make_unique<SurikireAudioProcessor>();
    ...
    void check6_full_state_roundtrip (SurikireAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<SurikireAudioProcessor>();
    auto& processor = *processorPtr;
    std::printf ("surikire preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + surikire_presets::bank.numPresets)
