//
// preset_test.cpp — wiring verification for the saturator's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < saturator_presets::kNumExclude; ++k)
            if (id == saturator_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (SaturatorAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (SaturatorAudioProcessor& p)
    {
    ...
    void check2_ids_exist (SaturatorAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = saturator_presets::bank;
    ...
    void check3_values_in_range (SaturatorAudioProcessor& p)
    {
    ...
    void check4_init_is_default (SaturatorAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (SaturatorAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<SaturatorAudioProcessor>();
    ...
    void check6_full_state_roundtrip (SaturatorAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<SaturatorAudioProcessor>();
    auto& processor = *processorPtr;

    std::printf ("saturator preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + saturator_presets::bank.numPresets)
