//
// preset_test.cpp — wiring verification for the vocal-mbcomp factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < vocal_mbcomp_presets::kNumExclude; ++k)
            if (id == vocal_mbcomp_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (VocalMbCompAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (VocalMbCompAudioProcessor& p)
    {
    ...
    void check2_ids_exist (VocalMbCompAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = vocal_mbcomp_presets::bank;
    ...
    void check3_values_in_range (VocalMbCompAudioProcessor& p)
    {
    ...
    void check4_init_is_default (VocalMbCompAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (VocalMbCompAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<VocalMbCompAudioProcessor>();
    ...
    void check6_full_state_roundtrip (VocalMbCompAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<VocalMbCompAudioProcessor>();
    auto& processor = *processorPtr;

    std::printf ("vocal-mbcomp preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + vocal_mbcomp_presets::bank.numPresets)
