//
// ClapEntry.cpp — the tn-equalizer CLAP plugin impl (IMPL_TARGET static library for
...
//   * factory_presets::PresetSession        — over the tn-equalizer bank + kExclude
...
//
// clap_plugin_descriptor id: jp.tatsunari-sounds.tn-equalizer (reverse-DNS).
//
#include <clap/clap.h>
...
namespace
{
    ...
    const clap_plugin_descriptor_t s_desc = {
        CLAP_VERSION_INIT,
        "jp.tatsunari-sounds.tn-equalizer",             // id (reverse-DNS)
        "TN Equalizer",                              // name
    ...
    //
    // tn-equalizer is a zero-prime core (primeFrames() == 0), so activate() never runs a prime
    ...
    // ── the shell Policy for tn-equalizer ───────────────────────────────────────
