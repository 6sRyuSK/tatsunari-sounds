//
// preset_test.cpp — wiring verification for the bus-compressor factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < bus_compressor_presets::kNumExclude; ++k)
            if (id == bus_compressor_presets::kExclude[k])
                return true;
        return false;
    }

    juce::RangedAudioParameter* rangedParam (BusCompressorAudioProcessor& p, const juce::String& id)
    {
    ...
    void check1_names (BusCompressorAudioProcessor& p)
    {
    ...
    void check2_ids_exist (BusCompressorAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = bus_compressor_presets::bank;
    ...
    void check3_values_in_range (BusCompressorAudioProcessor& p)
    {
    ...
    void check4_init_is_default (BusCompressorAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (BusCompressorAudioProcessor& p)
    {
    ...
    void check6_full_state_roundtrip (BusCompressorAudioProcessor& p)
    {
...
int main()
{
    ...
    if (processor.getNumPrograms() != 1 + bus_compressor_presets::bank.numPresets)
