//
// preset_test.cpp — wiring verification for mochi-stretch's factory presets.
//
...
namespace
{
    ...
    {
        for (int k = 0; k < mochi_stretch_presets::kNumExclude; ++k)
            if (id == mochi_stretch_presets::kExclude[k])
                return true;
        return false;
    }
    ...
    void check1_names (MochiStretchAudioProcessor& p)
    {
    ...
    void check2_ids_exist (MochiStretchAudioProcessor& p)
    {
        std::printf ("2. every preset paramID exists in the layout\n");
        const auto& bank = mochi_stretch_presets::bank;
    ...
    void check3_values_in_range (MochiStretchAudioProcessor& p)
    {
    ...
    void check4_init_is_default (MochiStretchAudioProcessor& p)
    {
    ...
    void check5_index_roundtrip (MochiStretchAudioProcessor& p)
    {
        ...
        auto restored = std::make_unique<MochiStretchAudioProcessor>();
        ...
        auto legacy = std::make_unique<MochiStretchAudioProcessor>();
        juce::MemoryBlock legacyState;
        if (auto xml = legacy->apvts.copyState().createXml())
            legacy->copyXmlToBinary (*xml, legacyState);
        auto legacyRestored = std::make_unique<MochiStretchAudioProcessor>();
    ...
    void check6_full_state_roundtrip (MochiStretchAudioProcessor& p)
    {
...
int main()
{
    ...
    auto processorPtr = std::make_unique<MochiStretchAudioProcessor>();
    auto& processor = *processorPtr;
    std::printf ("mochi-stretch preset wiring (%d programs)\n", processor.getNumPrograms());

    if (processor.getNumPrograms() != 1 + mochi_stretch_presets::bank.numPresets)
